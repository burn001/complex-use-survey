...
- **PART A (용도전환)**: 14개 분야 타당성(A1) · 방향분류(A2) · 완화 우선순위(A3, 순위매트릭스) · 재시공 유발 3분야 처리방향(A6, 배연 산정단위/차음 층간바닥/전용출입구 갈음) · 공개공지↔용적률 완화 활용(A7) · 경계값/누락 자유기술(A4·A5)
  - 2026-07-31 확장: 11 → 14개 분야(배연·제연, 차음, 전용출입구·동선 분리 신설 / 일조·인동간격 → 「대지·외부공간 기준」으로 확장 / 부대·복리시설 → 주거 부대설비 포함 두 트랙). PART A 안내에 「건축법」 제19조제7항 준용 구조 명시. **문항 배열 순서는 A1·A2·A3·A6·A7·A4·A5** (자유기술을 말미에 배치, ID는 신설 순서 유지)
